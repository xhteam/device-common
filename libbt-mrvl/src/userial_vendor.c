...
typedef struct
{
	...
	char port_type[VND_PORT_NAME_MAXLEN];
} vnd_userial_cb_t;
...
void userial_vendor_init(void)
{
    ...
    snprintf(vnd_userial.port_name, VND_PORT_NAME_MAXLEN, "%s", \
            BLUETOOTH_VENDOR_PORT);
	ALOGD("vnd_userial.port_name is %s ",vnd_userial.port_name);
...
int userial_vendor_open(tUSERIAL_CFG *p_cfg)
{
	...
	ALOGI("userial vendor open: opening %s[%s]", vnd_userial.port_name,
		vnd_userial.port_type);

	if(!strlen(vnd_userial.port_type)||
		!strcmp(vnd_userial.port_type,"h4")||
		!strcmp(vnd_userial.port_type,"uart")||
		!strcmp(vnd_userial.port_type,"uarth4")){
...
int userial_set_porttype(char *p_conf_name, char *p_conf_value, int param)
{
